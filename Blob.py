...
def calc_power(n_minor : int, ir: list, level, n_sims: int, exp_number: int, 
    ...
    for r in ir:
        ...
        s1, s2 = data_generator.blob()
        X = np.concatenate((s1, s2), axis=0)
        y = (torch.cat((torch.zeros(n_minor*r, 1), torch.ones(n_minor, 1)), 0)).squeeze(1).to(device, dtype).long()
        # data = data_generator.mixn_mixn(sigma0=0.8, sigma1=1, delta=0.1)
        # data = data_generator.mixn_sigmoid(sigma0=0.8, sigma1=0.5, delta=0.1)
        ...
        for sim in range(n_sims):
            ...
            'NN': NeuralNet(x_in=2, H=50, x_out=2, device=device, dtype=dtype)
            }
            y_np = y.to('cpu').numpy()
